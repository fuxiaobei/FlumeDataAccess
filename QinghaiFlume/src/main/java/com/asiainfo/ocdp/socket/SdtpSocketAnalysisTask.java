	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				logger.debug("get new buffer messages, length: " + preLen);
				...
				logger.debug(
						"events(records) size in current message body(default Maximum 40, which due to sdtp client)："
								+ ct);
				...
				if (msgType == 0x0001) {// 版本请求
					...
					ds.readFully(contextArr);
					msgQueue.offer(contextArr);
					// logger.info("msgType == 0x0005. get message body,
					// Content:" + new String(contextArr));
					
					if(!socket.isConnected()){
						logger.info("------------socket connection is closed!-----------");
					}else{
					...
					socket.close();
					logger.error("get wrong msgType: " + msgType);
