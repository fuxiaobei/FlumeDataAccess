...
/**
 * 青海自定义的MessagePreprocessor实现类.
 */
public class QinghaiMessagePreprocessor implements MessagePreprocessor {
	private static final Logger logger = LoggerFactory.getLogger(QinghaiMessagePreprocessor.class);
	...
	public String TimeStamp2Date(String timestampString) {
		...
		String date = new java.text.SimpleDateFormat("yyyyMMdd HH:mm:ss:SSS").format(new java.util.Date(timestamp));
		String now = new java.text.SimpleDateFormat("yyyyMMdd HH:mm:ss:SSS").format(new Date());
		logger.info("------mme start_time--------:" + date + ",curunt_time:" + now);
		return date;
	}
