...
public class FlumeSource extends AbstractSource implements Configurable,PollableSource {
    ...
    public Status process() {
            ...
            try {
                ...
                String[] messages = new String(msg).split(MESSAGE_SEPARATOR);
                //logger.info("come in for(),messages.count:"+messages.length);
                for (String signalling : messages){
                    flag++;
                    Map<String, String> headers = new HashMap();
                    //第6字段为imsi,作为key
                    String imsi = signalling.trim().split(HEADERS_KEY_SEPARATOR)[9];
                    try{
                ...
                logger.info("接收记录数：总共有sum:"+flag+"条记录，其中有效的有sum1:"+flage+"条，所耗时间为time:"+(endTime-startTime)+"ms");
                return Status.READY;
    ...
    private String getMessages(byte[] buffer, int off) {
        int len = off;
        //logger.info("getMessages come in buffer.count:" + buffer.length);
        StringBuilder sb = new StringBuilder();
        //循环遍历，取出多条记录数据
        for (; off < buffer.length; ) {
            //logger.info("for()..");
            int l = ConvToByte.byteToUnsignedShort(buffer, off);
            //logger.info("data len:" + ConvToByte.byteToUnsignedShort(buffer, off));
           // logger.info("care len:" + buffer[ConvToByte.byteToUnsignedShort(buffer, off)]);
            //logger.info("care len+1:" + buffer[ConvToByte.byteToUnsignedShort(buffer, off )+ 1]);
...
//            off += 1;
            ...
            epsBearerNum = epsBearerNum == 255?0:epsBearerNum;
            sb.append(epsBearerNum + "|");//EPS Bearer Number
            off += 1;
            //logger.info("epsBearerNum:"+epsBearerNum);
...
//
            ...
            off += 1;
            //logger.info("my off:"+off);
            sb.append("\n");
            //logger.info("a message end");
        }
        sb.deleteCharAt(sb.lastIndexOf("\n"));
        logger.info("getMessage end off:" + off);
        return sb.toString();
    }
//    private String getMessages(byte[] buffer,int off){
//        StringBuilder sb = new StringBuilder();
